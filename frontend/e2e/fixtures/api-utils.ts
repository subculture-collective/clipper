...
export async function createTestClip(
  ...
  const testClip = {
    ...
    url: clipData.url || `https://twitch.tv/test_channel/clip/clip${Date.now()}`,
 ...
 */
export async function deleteChannel(page: Page, channelId: string): Promise<Response> {
...
export async function waitForContentToLoad(
  ...
  try {
    await page.locator(selector).first().waitFor({ state: 'visible', timeout: maxWaitTime });
    return true;
  } catch (e) {
    return false;
  }
