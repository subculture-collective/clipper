...
export abstract class BasePage {
    ...
    async goto(options?: Parameters<Page['goto']>[1]): Promise<void> {
        ...
        // Dismiss consent banner if it appears (auto-consent may not be set)
        await this.dismissConsentBannerIfVisible();
    }
    ...
    async click(
        ...
        // Dismiss consent banner if visible, as it may block interactions
        await this.dismissConsentBannerIfVisible();
    ...
    async fillInput(selector: string | Locator, value: string): Promise<void> {
        // Dismiss consent banner if visible, as it may block form interactions
        await this.dismissConsentBannerIfVisible();
    ...
    /**
     * Dismiss the consent banner if it's visible
     * Handles both the simple view and detailed view of the consent banner
     */
    async dismissConsentBannerIfVisible(): Promise<void> {
        try {
            // Check if the consent banner is visible
            const consentBanner = this.page.locator(
            ...
            if (isVisible) {
                // Try to click "Accept All" button first (simpler, faster)
                const acceptAllButton = this.page
                    .locator('button:has-text("Accept All")')
                    .first();
                const acceptAllVisible = await acceptAllButton
                ...
                if (acceptAllVisible) {
                    ...
                    // If Accept All is not visible, try to close the detailed view first
                    const closeButton = this.page.locator(
                        'button[aria-label="Close detailed preferences"]',
                    );
            ...
            // Silently ignore consent banner dismissal errors
            // The banner should not block tests if auto-consent is working
        }
    }
}
