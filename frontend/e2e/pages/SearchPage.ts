...
export class SearchPage extends BasePage {
    ...
    constructor(page: Page) {
        ...
        this.searchInput = page
            .locator(
                ':is([data-testid="search-input"], input[type="search"], input[name="q"], input[placeholder*="search" i], [role="searchbox"])',
            )
            .first();
        ...
        // Suggestions/Autocomplete
        this.suggestionsContainer = page
            .locator(
                '[role="listbox"], [data-testid="suggestions"], .suggestions-dropdown',
            )
            .first();
        this.suggestionItems = this.suggestionsContainer.locator(
            '[role="option"], .suggestion-item, li',
        );
        this.suggestionsLoading = this.suggestionsContainer.locator(
        ...
        this.resultCards = page.locator(
            '[data-testid="clip-card"], .clip-card, .search-result-card',
        );
        ...
        // Filters
        this.filtersSection = page
            .locator('.filters, [data-testid="filters"]')
            .first();
        this.showFiltersButton = page
            .locator('button')
            .filter({ hasText: /show/i });
        this.clearFiltersButton = page
            .locator('button')
            .filter({ hasText: /clear.*all/i });
        this.languageFilters = this.filtersSection.locator(
            '.language-filter, [data-filter="language"]',
        );
        this.gameFilters = this.filtersSection.locator(
            '.game-filter, [data-filter="game"]',
        );
        this.dateRangeFilters = this.filtersSection.locator(
            '.date-range-filter, [data-filter="date"]',
        );
        this.tagFilters = this.filtersSection.locator(
            '.tag-filter, [data-filter="tags"]',
        );
        this.minVotesInput = this.filtersSection.locator(
            'input[name="minVotes"], input[placeholder*="votes" i]',
        );
        ...
        // Search History
        this.searchHistoryContainer = page
            .locator('.search-history, [data-testid="search-history"]')
            .first();
        this.searchHistoryItems = this.searchHistoryContainer.locator(
            'li, .history-item, button[data-testid^="history-item"]',
        );
        ...
        this.trendingSearchItems = this.trendingSearchesContainer.locator(
            'button[data-testid^="trending-search"]',
        );

        // Saved Searches
        this.savedSearchesContainer = page.getByTestId('saved-searches');
        this.savedSearchItems = this.savedSearchesContainer.locator(
            'div[data-testid^="saved-search"]',
        );
        this.clearSavedSearchesButton = page.getByTestId(
     ...
     */
    async switchTab(
        tab: 'all' | 'clips' | 'creators' | 'games' | 'tags',
    ): Promise<void> {
        const tabLocator = {
            all: this.allTab,
            clips: this.clipsTab,
    ...
    async getActiveTab(): Promise<string> {
        ...
        for (const tab of tabs) {
            const ariaSelected =
                await tab.locator.getAttribute('aria-selected');
    ...
    async filterByLanguage(language: string): Promise<void> {
        ...
        const languageOption = this.languageFilters
    ...
    async filterByGame(game: string): Promise<void> {
        await this.showFilters();
        const gameOption = this.gameFilters
            .locator('button, input')
            .filter({ hasText: new RegExp(game, 'i') });
    ...
    async filterByDateRange(
        ...
        await this.showFilters();
        const dateOption = this.dateRangeFilters
            .locator('button, input')
            .filter({
    ...
    async goToNextPage(): Promise<void> {
        ...
        const targetPage =
            Number(currentUrl.searchParams.get('page') || '1') + 1;
        await this.nextPageButton.click();
        await this.page.waitForURL(
            url => url.searchParams.get('page') === String(targetPage),
        );
    ...
    async goToPreviousPage(): Promise<void> {
        ...
        await this.page.waitForURL(
            url =>
                ...
                (!url.searchParams.get('page') && targetPage === 1),
        );
        await this.page.waitForFunction(expected => {
            const el = document.querySelector('[data-testid="page-number"]');
            return el?.textContent?.includes(`Page ${expected}`);
    ...
    async getSearchParams(): Promise<URLSearchParams> {
        ...
        try {
            const stored = await this.page.evaluate(() =>
                sessionStorage.getItem('search:lastParams'),
            );
            if (stored) {
    ...
    async getTrendingSearches(): Promise<string[]> {
        await this.trendingSearchesContainer.waitFor({
            state: 'visible',
            timeout: 5000,
        });
    ...
    async getSavedSearches(): Promise<string[]> {
        try {
            await this.savedSearchesContainer.waitFor({
                state: 'visible',
                timeout: 2000,
            });
    ...
    async deleteSavedSearch(index: number): Promise<void> {
        const deleteButton = this.savedSearchItems
            .nth(index)
            .locator('button[data-testid^="delete-saved-search"]');
