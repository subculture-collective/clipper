...
export class SearchPage extends BasePage {
    ...
    constructor(page: Page) {
        ...
        this.searchHistoryItems =
            this.searchHistoryContainer.locator('li, .history-item');
        this.clearHistoryButton = page
            .locator('button')
            .filter({ hasText: /clear.*history/i });
    }
    ...
    async getResultCount(): Promise<number> {
        ...
        // Text-based fallback for webkit compatibility
        const parseCountFromText = async (): Promise<number | null> => {
            const countElement = this.page
                .locator('text=/Found \\d+ results/i')
                .first();
            try {
                await countElement.waitFor({ state: 'visible', timeout: 2000 });
        ...
        // Fallback 1: text-based search for webkit
        const textCount = await parseCountFromText();
        if (textCount !== null) {
            return textCount;
        }

        // Fallback 2: wait for visible cards and count them
        try {
        ...
        // Retry text-based search
        const retryTextCount = await parseCountFromText();
        if (retryTextCount !== null) {
            return retryTextCount;
        }
