...
export async function fillStripeCheckoutForm(
    ...
    if (email) {
        ...
        try {
            ...
            console.debug(
                'Stripe checkout: Email field not visible or required:',
                error,
            );
        }
    }
    ...
    const cardNumberField = page
        .frameLocator(STRIPE_IFRAME_SELECTORS.cardNumber)
        .locator('input[name="cardnumber"], input[placeholder*="card number"]');
    await cardNumberField.fill(cardNumber);

    // Fill expiry
    const expiryField = page
        .frameLocator(STRIPE_IFRAME_SELECTORS.expiry)
        .locator('input[name="exp-date"], input[placeholder*="MM"]');
    await expiryField.fill(expiry);

    // Fill CVC
    const cvcField = page
        .frameLocator(STRIPE_IFRAME_SELECTORS.cvc)
        .locator('input[name="cvc"], input[placeholder*="CVC"]');
    await cvcField.fill(cvc);

    // Fill ZIP if visible
    const zipField = page.locator(
        'input[name="postal"], input[name="zip"], input[placeholder*="ZIP"]',
    );
    try {
        ...
        console.debug(
            'Stripe checkout: ZIP field not visible or required:',
            error,
        );
    }
}
...
export async function completeStripeCheckout(
    ...
    if (expectSuccess) {
        ...
        await page
            .locator('text=/declined|failed|error/i')
...
export async function sendMockWebhook(
    ...
    if (!isTestEnv) {
        throw new Error(
            'sendMockWebhook should only be used in test environments. Set NODE_ENV=test or PLAYWRIGHT_TEST=true.',
        );
    }
...
export async function verifyStripeTestMode(page: Page) {
    ...
    const testModeIndicator = page.locator(
        'text=/test.*mode|test.*environment/i',
    );
...
export function getStripePriceIds() {
    return {
        monthly:
            ...
            TEST_PRICE_IDS.monthly,
        yearly:
            process.env.VITE_STRIPE_PRO_YEARLY_PRICE_ID ||
            TEST_PRICE_IDS.yearly,
    };
}
...
export async function verifyProFeaturesEnabled(page: Page) {
    ...
    const paywall = page.locator(
        '[data-testid="paywall-modal"], text=/upgrade.*to.*pro/i',
    );
...
export async function verifyProFeaturesDisabled(page: Page) {
    ...
    const upgradePrompt = page
        .locator('text=/upgrade|subscribe|get.*pro/i')
        .first();
