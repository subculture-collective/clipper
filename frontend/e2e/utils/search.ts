...
export async function performSearch(
    ...
    if (filters) {
        ...
        if (filters.minVotes !== undefined)
            params.append('min_votes', filters.minVotes.toString());
 ...
 */
export async function performSearchAPI(
    page: Page,
    options: SearchOptions,
): Promise<any> {
...
async function extractSearchResults(page: Page): Promise<SearchResult[]> {
    ...
    await page
        .waitForSelector(
            '[data-testid="clip-card"], .search-result-card, .empty-state',
            {
                timeout: DEFAULT_SEARCH_TIMEOUT,
            },
        )
    ...
    for (let i = 0; i < count; i++) {
        ...
        const titleElement = card
            .locator('h2, h3, .title, [data-testid="clip-title"]')
            .first();
 ...
 */
export function verifySearchRelevance(
...
export async function applyAndVerifyFilters(
    ...
    if (filters.minVotes !== undefined)
        params.set('min_votes', filters.minVotes.toString());
    ...
    if (filters.language && newParams.get('language') !== filters.language)
        allMatch = false;
    if (gameFilter && newParams.get('game_id') !== gameFilter) allMatch = false;
    if (
        filters.minVotes !== undefined &&
        newParams.get('min_votes') !== filters.minVotes.toString()
    )
        allMatch = false;
...
export async function measureSuggestionLatency(
    ...
    for (let i = 0; i < iterations; i++) {
        ...
        const searchInput = page
            .locator(
                '[data-testid="search-input"], input[type="search"], input[name="q"], input[placeholder*="search" i]',
            )
            .first();
        ...
        try {
            await page
                .locator('[role="listbox"], [data-testid="suggestions"]')
                .first()
                .waitFor({
                    state: 'visible',
...
export function verifySuggestionQuality(
    ...
    const relevantCount = suggestions.filter(
        s =>
...
export async function measureSearchLatency(
    ...
    for (let i = 0; i < iterations; i++) {
        ...
        const searchInput = page
            .locator(
                '[data-testid="search-input"], input[type="search"], input[name="q"]',
            )
            .first();
        ...
        await page.waitForSelector(
            '[data-testid="results-count"], [data-testid="clip-card"], .search-result-card, [data-testid="empty-state"]',
            {
                timeout: DEFAULT_SEARCH_TIMEOUT,
            },
        );
 ...
 */
export async function cleanupSearchData(
    page: Page,
    clipIds: string[],
): Promise<void> {
    const apiUrl = getApiBaseUrl();
