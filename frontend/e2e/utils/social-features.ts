...
export async function createComment(
    ...
    try {
        const { ok, json } = await fetchJson(
            page,
            `${apiUrl}/clips/${commentData.clipId}/comments`,
            {
                method: 'POST',
                headers: {
...
export async function getComments(
    ...
    try {
        const { ok, json } = await fetchJson(
            page,
            `${apiUrl}/clips/${clipId}/comments?${params}`,
        );
...
export async function voteOnComment(
    ...
    try {
        const { ok, json } = await fetchJson(
            page,
            `${apiUrl}/comments/${commentId}/vote`,
            {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
 ...
 */
export async function removeClipVote(
    page: Page,
    clipId: string,
): Promise<void> {
    const apiUrl = getApiBaseUrl();
 ...
 */
export async function getClipVoteStatus(
    page: Page,
    clipId: string,
): Promise<{ user_vote: number | null; vote_score: number }> {
    const apiUrl = getApiBaseUrl();
    const token = await getAuthToken(page);
...
export async function followUser(
    ...
    try {
        const { ok, json } = await fetchJson(
            page,
            `${apiUrl}/users/${targetUserId}/follow`,
            {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
...
export async function getFollowingStatus(
    ...
    try {
        const { ok, json } = await fetchJson(
            page,
            `${apiUrl}/users/${targetUserId}/follow-status`,
            {
                headers: {
                    ...(token ? { Authorization: `Bearer ${token}` } : {}),
                },
...
export async function addClipsToPlaylist(
    ...
    try {
        const { ok, json } = await fetchJson(
            page,
            `${apiUrl}/playlists/${playlistId}/clips`,
            {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
...
export async function removeClipFromPlaylist(
    ...
    try {
        const { ok } = await fetchJson(
            page,
            `${apiUrl}/playlists/${playlistId}/clips/${clipId}`,
            {
                method: 'DELETE',
                headers: {
                    ...(token ? { Authorization: `Bearer ${token}` } : {}),
...
export async function getPlaylistShareLink(
    ...
    try {
        const { ok, json } = await fetchJson(
            page,
            `${apiUrl}/playlists/${playlistId}/share`,
            {
                headers: {
                    ...(token ? { Authorization: `Bearer ${token}` } : {}),
                },
 ...
 */
export async function validatePlaylistAccess(
    page: Page,
    playlistId: string,
...
export async function blockUser(
    ...
    try {
        const { ok, json } = await fetchJson(
            page,
            `${apiUrl}/users/${targetUserId}/block`,
            {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
...
export async function isUserBlocked(
    ...
    try {
        const { ok, json } = await fetchJson(
            page,
            `${apiUrl}/users/${targetUserId}/block-status`,
            {
                headers: {
                    ...(token ? { Authorization: `Bearer ${token}` } : {}),
                },
...
export async function triggerRateLimit(
    ...
    for (let i = 0; i < attempts; i++) {
        try {
            ...
            if (
                lastResponse?.error?.includes('rate limit') ||
 ...
 */
export async function verifyRateLimitMessage(
    page: Page,
            ...
            if (await element.isVisible({ timeout: 2000 }).catch(() => false)) {
                if (expectedMessage) {
                    ...
                    return (
                        text
                            ?.toLowerCase()
                            .includes(expectedMessage.toLowerCase()) || false
                    );
                }
                return true;
            }
 ...
 */
export async function waitForRateLimitClear(
    page: Page,
...
export async function createWatchParty(
    ...
    try {
        ...
        if (!ok) {
            console.warn(
                'Failed to create watch party via API, using mock data',
            );
...
export async function joinWatchParty(
    ...
    try {
        const { ok, json } = await fetchJson(
            page,
            `${apiUrl}/watch-parties/${partyIdOrCode}/join`,
            {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
...
export async function leaveWatchParty(
    ...
    try {
        const { ok } = await fetchJson(
            page,
            `${apiUrl}/watch-parties/${partyId}/leave`,
            {
                method: 'DELETE',
                headers: {
                    ...(token ? { Authorization: `Bearer ${token}` } : {}),
 ...
 */
export async function getWatchPartyParticipants(
    page: Page,
    partyId: string,
    ...
    try {
        const { ok, json } = await fetchJson(
            page,
            `${apiUrl}/watch-parties/${partyId}/participants`,
            {
                headers: {
                    ...(token ? { Authorization: `Bearer ${token}` } : {}),
                },
...
export async function updateWatchPartySettings(
    page: Page,
    partyId: string,
    settings: Partial<WatchPartyData>,
): Promise<any> {
    const apiUrl = getApiBaseUrl();
    const token = await getAuthToken(page);
    ...
    try {
        const { ok, json } = await fetchJson(
            page,
            `${apiUrl}/watch-parties/${partyId}/settings`,
            {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
        ...
        console.warn(
            'API not available for updating watch party settings:',
            error,
        );
        return { success: false };
    }
 ...
 */
export async function kickWatchPartyParticipant(
    page: Page,
    partyId: string,
    ...
    try {
        const { ok, json } = await fetchJson(
            page,
            `${apiUrl}/watch-parties/${partyId}/participants/${participantId}`,
            {
                method: 'DELETE',
                headers: {
                    ...(token ? { Authorization: `Bearer ${token}` } : {}),
 ...
 */
export async function deleteWatchParty(
    page: Page,
    partyId: string,
): Promise<void> {
    const apiUrl = getApiBaseUrl();
