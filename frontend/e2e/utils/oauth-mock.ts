...
async function registerOAuthRoute(
    page: Page,
...
export function generateMockUser(
    overrides: Partial<MockOAuthUser> = {},
): MockOAuthUser {
    return {
        id: `user_${Date.now()}`,
...
export async function mockOAuthSuccess(
    ...
    // Block test-login to prevent auto-login from interfering with OAuth flow tests
    await page.route('**/api/v1/auth/test-login', async (route: Route) => {
        await route.fulfill({
            ...
            body: JSON.stringify({
                success: false,
                error: 'Test login disabled for OAuth tests',
            }),
        });
    });
...
export async function mockOAuthAbort(page: Page): Promise<void> {
    ...
    await registerOAuthRoute(page, async (route: Route) => {
        ...
        await page
            .goto(callbackUrl, { waitUntil: 'networkidle' })
            .catch(() => {});
    });
...
export async function mockOAuthInvalidState(page: Page): Promise<void> {
    ...
    await registerOAuthRoute(page, async (route: Route) => {
        ...
        await page
            .goto(
                `/login?oauth_error=invalid_state&error_description=${encodeURIComponent('State parameter validation failed')}&state=${badState}`,
