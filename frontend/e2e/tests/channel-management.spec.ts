...
async function setupChannelApiMocks(page: import('@playwright/test').Page) {
    ...
    await page.route('**/api/v1/chat/channels**', async route => {
        ...
        if (method === 'GET' && sub === 'members') {
            const members = [
                { user_id: 'user_e2e', username: 'user1_e2e', role: 'owner' },
            ];
            await route.fulfill({
                status: 200,
                contentType: 'application/json',
        ...
        if (sub === 'members' && sub2) {
            if (method === 'DELETE' || method === 'PATCH') {
                await route.fulfill({
                    status: 403,
                    contentType: 'application/json',
                    body: JSON.stringify({ error: 'owner cannot be modified' }),
                });
                return;
            }
        }
...
test.describe('Channel Management', () => {
    ...
    test('should create a channel and assign creator as owner', async ({
        ...
        const roleResponse = await apiRequest(
            page,
            `/api/v1/chat/channels/${testChannelId}/role`,
        );
    ...
    test('should navigate to channel settings and display current user role', async ({
        ...
        // Check that the page shows the user's role (use first() to handle multiple matches)
        await expect(page.getByText(/Your role:/i).first()).toBeVisible();
    ...
    test('should display member list with roles', async ({ page }) => {
        ...
        // Check for members section (use first() to handle multiple matches)
        await expect(
            page.getByRole('heading', { name: /members/i }).first(),
        ).toBeVisible();

        // Should show at least 1 member (the owner)
        await expect(page.getByText(/Members \(1\)/i).first()).toBeVisible();
    });
    ...
    test('owner should see invite and danger zone controls', async ({
        ...
        // Use first() to avoid strict mode violation when there are multiple matching buttons
        await expect(
            page.getByRole('button', { name: /invite member/i }).first(),
        ).toBeVisible();

        // Owner should see danger zone - use first() to avoid strict mode violation
        await expect(
            page.getByRole('heading', { name: /danger zone/i }).first(),
        ).toBeVisible();
        await expect(
            page.getByRole('button', { name: /delete channel/i }).first(),
        ).toBeVisible();
    });
    ...
    test('should prevent removing or demoting the owner', async ({ page }) => {
        ...
        const membersResponse = await apiRequest(
            page,
            `/api/v1/chat/channels/${testChannelId}/members`,
        );
        const membersData = membersResponse.json;
        const ownerMember = membersData.members.find(
    ...
    test('owner should be able to delete channel', async ({ page }) => {
        ...
        const deleteResponse = await apiRequest(
            page,
            `/api/v1/chat/channels/${testChannelId}`,
            { method: 'DELETE' },
        );
    ...
    test.skip('admin should be able to remove members but not owner', async ({
        ...
        const removeResponse = await apiRequest(
            adminPage,
            `/api/v1/chat/channels/${channelId}/members/${memberContext.user.id}`,
            {
                method: 'DELETE',
            },
        );
        ...
        const removeOwnerResponse = await apiRequest(
            adminPage,
            `/api/v1/chat/channels/${channelId}/members/${ownerPage.user?.id || 'owner-id'}`,
            {
                method: 'DELETE',
            },
        );
...
test.describe('Channel Member Permissions', () => {
    test.skip('member should not see admin controls', async ({
        ...
        const adminControls = memberPage.locator(
            '[data-testid="admin-controls"], .admin-panel',
        );
    ...
    test.skip('moderator should not be able to update roles', async ({
        ...
        const updateResponse = await apiRequest(
            modPage,
            `/api/v1/chat/channels/${channel.id}/members/${targetContext.user.id}`,
            {
                method: 'PATCH',
                data: { role: 'moderator' },
            },
