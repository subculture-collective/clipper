...
async function setupModerationMocks(page: Page) {
  ...
  await page.context().route('**/api/**', async (route: Route, request: Request) => {
    ...
    if (removeModerator && method === 'DELETE') {
      ...
      if (!moderator) {
        return respond(route, 404, { error: 'Moderator not found' });
      }
      ...
      // Create audit log
      createAuditLog('remove_moderator', 'moderator', moderatorId, currentUser.id, {
    ...
    if (pathname === '/chat/sync-bans' && method === 'POST') {
      ...
      // Create audit log
      createAuditLog('sync_bans', 'channel', body.channel_id || 'default', currentUser.id, {
    ...
    // Check sync progress
    const syncProgress = pathname.match(/^\/chat\/sync-bans\/([^/]+)\/progress$/);
    if (syncProgress && method === 'GET') {
      ...
      return respond(route, 200, {
        ...
        message: 'Twitch bans have been synchronized to your channel.',
      });
    }
...
test.describe('Moderation E2E', () => {
  test.describe('Moderator Onboarding Flow', () => {
    test('admin creates new moderator and grants permissions', async ({ page }) => {
      ...
      await expect(
        page.locator('[role="alert"]').filter({ hasText: /success|added/i })
      ).toBeVisible({ timeout: 5000 });

      // Verify audit log was created
      const logs = mocks.getAuditLogs();
      const createLog = logs.find(
        (log) => log.action === 'create_moderator' && log.details?.user_id === 'user-1'
      );
    ...
    test('moderator can access moderation features after being granted permissions', async ({
      ...
      await expect(page.getByText(/403|access denied|forbidden/i)).not.toBeVisible();
    });

    test('regular user cannot access moderation features', async ({ page }) => {
      const mocks = await setupModerationMocks(page);
      ...
      await expect(page.getByText(/403|access denied|forbidden/i)).toBeVisible({
        timeout: 5000,
      });
    });
  });
  ...
  test.describe('Ban Sync Flow', () => {
    ...
    test('verify bans appear in ban list after sync', async ({ page }) => {
      ...
      await expect(page.getByText('banneduser1')).toBeVisible({ timeout: 5000 });
      await expect(page.getByText('banneduser2')).toBeVisible({ timeout: 5000 });
  ...
  test.describe('Audit Log Verification', () => {
    test('audit logs show all moderation actions', async ({ page }) => {
      ...
      await expect(page.getByText(/create.*moderator/i)).toBeVisible({ timeout: 5000 });

      // Check for sync_bans action
      await expect(page.getByText(/sync.*ban/i)).toBeVisible({ timeout: 5000 });
    });
    ...
    test('audit log filtering by action type', async ({ page }) => {
      ...
      const filterSelect = page.getByLabel(/action.*type/i).or(page.getByLabel(/filter/i));

      if (await filterSelect.isVisible({ timeout: 2000 }).catch(() => false)) {
        await filterSelect.click();

        // Select specific action type
        const option = page.getByRole('option', { name: /sync.*ban/i }).or(
          page.getByText(/sync.*ban/i).filter({ has: page.locator('input[type="checkbox"]') })
        );
    ...
    test('audit log details modal displays complete information', async ({ page }) => {
      const mocks = await setupModerationMocks(page);
      ...
      const logEntry = page.locator('[data-testid^="audit-log-"]').first().or(
        page.getByText(/sync.*ban/i).first()
      );
  ...
  test.describe('Banned User Interactions', () => {
    test('banned user sees disabled interaction buttons on posts', async ({ page }) => {
      const mocks = await setupModerationMocks(page);
      ...
      await expect(
        page.getByText(/banned|restricted|suspended/i).filter({ hasText: /cannot/i })
      ).toBeVisible({ timeout: 5000 });
    });

    test('unbanned user regains full interaction capabilities', async ({ page }) => {
      const mocks = await setupModerationMocks(page);
    ...
    test('moderator can revoke ban and user is immediately unrestricted', async ({ page }) => {
      const mocks = await setupModerationMocks(page);
      ...
      const revokeButton = page
        ...
        .or(page.getByRole('button', { name: /revoke|unban/i }).first());
      ...
      if (await revokeButton.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        // Wait for success message
        await expect(
          page.locator('[role="alert"]').filter({ hasText: /success|unbanned|revoked/i })
        ).toBeVisible({ timeout: 5000 });

        // Verify audit log was created
        const logs = mocks.getAuditLogs();
        const unbanLog = logs.find((log) => log.action === 'unban_user' && log.resource_id === 'ban-1');
  ...
  test.describe('Permission Enforcement', () => {
    ...
    test('non-admins cannot manage moderators', async ({ page }) => {
      ...
      const isAccessDenied = await accessDenied.isVisible({ timeout: 2000 }).catch(() => false);
      const isAddButtonHidden = !(await addButton.isVisible({ timeout: 2000 }).catch(() => false));

      expect(isAccessDenied || isAddButtonHidden).toBe(true);
    });

    test('moderators can view audit logs but not manage moderators', async ({ page }) => {
      const mocks = await setupModerationMocks(page);
  ...
  test.describe('Error Handling and Edge Cases', () => {
    test('handles network errors gracefully during ban sync', async ({ page }) => {
      const mocks = await setupModerationMocks(page);
      ...
      if (await syncButton.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        if (await modal.first().isVisible({ timeout: 2000 }).catch(() => false)) {
          ...
          // Verify error message is displayed
          await expect(
            page.locator('[role="alert"]').filter({ hasText: /error|failed|network/i })
          ).toBeVisible({ timeout: 5000 });
        }
      }
    });
    ...
    test('handles empty ban list gracefully', async ({ page }) => {
      ...
      await expect(
        page.getByText(/no bans|empty|no results/i).filter({ hasNotText: /found|matching/ })
      ).toBeVisible({ timeout: 5000 });
    });

    test('handles concurrent moderator actions without conflicts', async ({ page }) => {
      const mocks = await setupModerationMocks(page);
      ...
      const logs = mocks.getAuditLogs();
      const moderatorLogs = logs.filter((log) => log.action === 'create_moderator');
  ...
  test.describe('Performance and Browser Compatibility', () => {
    test('ban list loads within acceptable time', async ({ page }) => {
      ...
      await expect(page.getByText('user1')).toBeVisible({ timeout: 5000 });
