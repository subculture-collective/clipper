...
async function setupTwitchModerationMocks(page: Page) {
  ...
  const bannedUsers = new Map<string, { banId: string; reason: string; expiresAt?: string }>();
  ...
  await page.context().route('**/api/**', async (route: Route, request: Request) => {
    ...
    if (pathname === '/moderation/twitch/ban' && method === 'POST') {
      ...
      const body = (request.postDataJSON?.() || {}) as TwitchBanRequest;

      // Site moderators are read-only
      if (currentUser.role === 'moderator' && !currentUser.is_twitch_moderator) {
      ...
      if (!currentUser.is_broadcaster && !currentUser.is_twitch_moderator) {
        return respond(route, 403, {
          error: 'Forbidden',
      ...
      // Create audit log
      createAuditLog('twitch_ban_user', 'twitch_ban', banId, currentUser.id, {
    ...
    if (pathname === '/moderation/twitch/ban' && method === 'DELETE') {
      ...
      if (banInfo) {
        ...
        // Create audit log
        createAuditLog('twitch_unban_user', 'twitch_ban', banInfo.banId, currentUser.id, {
...
test.describe('Twitch Ban/Unban Actions E2E', () => {
  test.describe('Broadcaster Ban Operations', () => {
    test('broadcaster can permanently ban user with reason', async ({ page }) => {
      ...
      const banButton = page.getByRole('button', { name: /ban.*twitch/i }).first();
      ...
      const permanentRadio = page.getByLabel(/permanent/i).or(page.getByText(/permanent/i));
      if (await permanentRadio.isVisible({ timeout: 2000 }).catch(() => false)) {
        await permanentRadio.click();
      }

      // Confirm ban
      const confirmButton = page
        .getByRole('button', { name: /confirm|ban/i })
        .filter({ hasNotText: /cancel/i });
      await confirmButton.click();

      // Verify success message
      await expect(
        page.locator('[role="alert"]').filter({ hasText: /success|banned/i })
      ...
      expect(banLog?.details?.reason).toBe('Violation of community guidelines');
    ...
    test('broadcaster can timeout user with duration', async ({ page }) => {
      ...
      if (await banButton.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        const timeoutRadio = page.getByLabel(/timeout/i).or(page.getByText(/timeout/i));
        if (await timeoutRadio.isVisible({ timeout: 2000 }).catch(() => false)) {
          await timeoutRadio.click();
        }

        // Set duration (600 seconds = 10 minutes)
        const durationInput = page.getByLabel(/duration/i).or(page.getByPlaceholder(/duration|seconds/i));
        if (await durationInput.isVisible({ timeout: 2000 }).catch(() => false)) {
          await durationInput.fill('600');
        }

        const confirmButton = page
          .getByRole('button', { name: /confirm|ban|timeout/i })
          .filter({ hasNotText: /cancel/i });
        await confirmButton.click();

        await expect(
          page.locator('[role="alert"]').filter({ hasText: /success|timeout/i })
        ).toBeVisible({ timeout: 5000 });
    ...
    test('broadcaster can unban user', async ({ page }) => {
      ...
      const unbanButton = page.getByRole('button', { name: /unban.*twitch/i }).first();
      if (await unbanButton.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        await expect(
          page.locator('[role="alert"]').filter({ hasText: /success|unbanned/i })
        ).toBeVisible({ timeout: 5000 });

        const logs = mocks.getAuditLogs();
        const unbanLog = logs.find((log) => log.action === 'twitch_unban_user');
  ...
  test.describe('Site Moderator Read-Only Enforcement', () => {
    test('site moderator cannot ban user on Twitch', async ({ page }) => {
      ...
      const isVisible = await banButton.isVisible({ timeout: 2000 }).catch(() => false);
    ...
    test('site moderator cannot unban user on Twitch', async ({ page }) => {
      ...
      const unbanButton = page.getByRole('button', { name: /unban.*twitch/i }).first();
      const isVisible = await unbanButton.isVisible({ timeout: 2000 }).catch(() => false);
  ...
  test.describe('Error Handling', () => {
    test('shows error when user lacks Twitch OAuth scopes', async ({ page }) => {
      const mocks = await setupTwitchModerationMocks(page);
      ...
      if (await banButton.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        if (await modal.first().isVisible({ timeout: 2000 }).catch(() => false)) {
          ...
          await expect(
            page.locator('[role="alert"]').filter({ 
              hasText: /insufficient.*scope|re.*authenticate|oauth/i 
            })
          ).toBeVisible({ timeout: 5000 });
        }
      }
    ...
    test('shows error when user not authenticated with Twitch', async ({ page }) => {
      const mocks = await setupTwitchModerationMocks(page);
      ...
      if (await banButton.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        if (await modal.first().isVisible({ timeout: 2000 }).catch(() => false)) {
          ...
          await expect(
            page.locator('[role="alert"]').filter({ 
              hasText: /not authenticated|authenticate.*twitch/i 
            })
          ).toBeVisible({ timeout: 5000 });
        }
      }
    ...
    test('shows error when rate limit exceeded', async ({ page }) => {
      ...
      if (await banButton.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        if (await modal.first().isVisible({ timeout: 2000 }).catch(() => false)) {
          ...
          await expect(
            page.locator('[role="alert"]').filter({ 
              hasText: /rate limit|try again later/i 
            })
          ).toBeVisible({ timeout: 5000 });
        }
      }
  ...
  test.describe('Audit Logging', () => {
    ...
    test('creates audit log for unban action', async ({ page }) => {
      ...
      await page.request.delete('/api/v1/moderation/twitch/ban?broadcasterID=12345&userID=99999');
