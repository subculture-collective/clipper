...
type ClipSubmissionMockServer = {
    ...
    setRateLimit(force: boolean): void;
    seedSubmissions(
        userId: string,
        count: number,
        options?: {
...
async function setupClipSubmissionApiMocks(
    ...
    const createSubmissionRecord = (
        data: Partial<ClipSubmissionRecord> & {
            clip_url: string;
            title?: string;
            user_id: string;
        },
        ...
        return {
            id: `mock-submission-${Date.now()}-${Math.random()
                .toString(16)
                .slice(2)}`,
    ...
    await page
        ...
        .route('**/api/**', async (route: Route, request: Request) => {
            ...
            if (pathname === '/submissions' && method === 'POST') {
                ...
                if (forceRateLimit || userSubs.length >= 10) {
                    // Align mock with frontend rate limit handling shape
                    const retryAfter = Math.floor(Date.now() / 1000) + 60; // 1 minute cooldown for tests
            ...
            if (pathname === '/admin/submissions' && method === 'GET') {
                ...
                const slice = values
                    .slice((pageParam - 1) * limit, pageParam * limit)
                    .map(s => ({
                        ...s,
    ...
    return {
        ...
        seedSubmissions: async (
            ...
            for (let i = 0; i < count; i++) {
                const record = createSubmissionRecord({
                    clip_url:
                        options?.clipUrl ||
                        `https://clips.twitch.tv/test-${Date.now()}-${i}`,
        ...
        setRateLimit: (force: boolean) => {
            forceRateLimit = force;
        },
    };
}
...
test.describe('Clip Submission E2E Flow', () => {
    test.describe('Authentication & Authorization', () => {
        test('unauthenticated user sees login prompt', async ({
            submitClipPage,
        }) => {
    ...
    test.describe('Scenario 2: Rate Limiting', () => {
        test('rate limiting prevents excessive submissions', async ({
            ...
            // Persist an active rate limit to mirror backend response and ensure UI shows warning immediately
            ...
            // Then: rate limit error shown immediately
    ...
    test.describe('Scenario 4: Admin Approval Flow', () => {
        test('admin can approve submission', async ({
            ...
            const hasPendingSubmissions =
                await adminModerationPage.getPendingSubmissionsCount();
