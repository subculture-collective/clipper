...
function TestConsumer() {
  ...
  return (
    <div>
      ...
      <div data-testid="personalized-ads">{String(consent.personalizedAds)}</div>
      <div data-testid="performance">{String(consent.performance)}</div>
      <div data-testid="can-personalized">{String(canShowPersonalizedAds)}</div>
      ...
      <button onClick={resetConsent} data-testid="reset">Reset</button>
      <button 
...
describe('ConsentContext', () => {
  ...
  it('should default to privacy-preserving settings', () => {
    ...
    expect(screen.getByTestId('personalized-ads')).toHaveTextContent('false');
  ...
  it('should accept all when clicking accept all', () => {
    ...
    expect(screen.getByTestId('personalized-ads')).toHaveTextContent('true');
  ...
  it('should persist consent to localStorage', () => {
    ...
    expect(parsed.preferences.personalizedAds).toBe(true);
  });

  it('should load consent from localStorage', () => {
    // Pre-set consent in localStorage
    localStorage.setItem('clipper_consent_preferences', JSON.stringify({
      version: '1.0',
      preferences: {
        essential: true,
        analytics: true,
  ...
  it('should disallow personalized ads when DNT is enabled even with consent', () => {
    ...
    expect(screen.getByTestId('personalized-ads')).toHaveTextContent('true');
    expect(screen.getByTestId('can-personalized')).toHaveTextContent('false');
  ...
  it('should allow personalized ads when consented and no DNT', () => {
    ...
    expect(screen.getByTestId('can-personalized')).toHaveTextContent('true');
