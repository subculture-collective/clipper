...
describe('CommentSection', () => {
    ...
    describe('Loading and error states', () => {
        it('should show loading spinner while fetching comments', () => {
            vi.mocked(commentApi.fetchComments).mockImplementation(() => new Promise(() => { }) // Promise that never resolves to simulate infinite loading
            );
            renderWithClient(_jsx(CommentSection, { clipId: "clip-1" }));
    ...
    describe('Props handling', () => {
        it('should pass currentUserId to CommentItem components', async () => {
            ...
            vi.mocked(commentApi.fetchComments).mockResolvedValue(mockResponse);
            renderWithClient(_jsx(CommentSection, { clipId: "clip-1", currentUserId: "user-1" }));
            await waitFor(() => {
                expect(screen.getByText(/Test comment/i)).toBeInTheDocument();
            });
        ...
        it('should pass isAdmin to CommentItem components', async () => {
            ...
            vi.mocked(commentApi.fetchComments).mockResolvedValue(mockResponse);
            renderWithClient(_jsx(CommentSection, { clipId: "clip-1", currentUserId: "user-1", isAdmin: true }));
            await waitFor(() => {
                expect(screen.getByText(/Test comment/i)).toBeInTheDocument();
            });
        ...
        it('should apply custom className', async () => {
            ...
            vi.mocked(commentApi.fetchComments).mockResolvedValue(mockResponse);
            const { container } = renderWithClient(_jsx(CommentSection, { clipId: "clip-1", className: "custom-class" }));
            await waitFor(() => {
                expect(screen.getByText(/Comments \(0\)/i)).toBeInTheDocument();
            });
