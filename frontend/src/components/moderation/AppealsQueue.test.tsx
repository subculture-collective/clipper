...
vi.mock('./AppealResolutionModal', () => ({
  AppealResolutionModal: ({ open, appeal, onClose, onSuccess }: any) => {
    if (!open) return null;
    return (
...
describe('AppealsQueue', () => {
  ...
  describe('Rendering', () => {
    ...
    it('displays empty state when no appeals', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('No pending appeals found')).toBeInTheDocument();
      });
    });
  });
  ...
  describe('Status Filtering', () => {
    it('loads pending appeals by default', async () => {
      ...
      await waitFor(() => {
        expect(moderationApi.getAdminAppeals).toHaveBeenCalledWith('pending');
      });
    });
    ...
    it('uses initialStatus prop when provided', async () => {
      ...
      await waitFor(() => {
        expect(moderationApi.getAdminAppeals).toHaveBeenCalledWith('approved');
      });
    });
    ...
    it('filters by rejected status when button clicked', async () => {
      ...
      await waitFor(() => {
        expect(moderationApi.getAdminAppeals).toHaveBeenCalledWith('rejected');
      });
    });
  ...
  describe('Appeal Details', () => {
    it('displays appeal information', async () => {
      ...
      expect(screen.getByText('appeal-1', { exact: false })).toBeInTheDocument();
      expect(screen.getByText('I did not violate any rules')).toBeInTheDocument();
      expect(screen.getByText('ban', { exact: false })).toBeInTheDocument();
      expect(screen.getByText('Spam', { exact: false })).toBeInTheDocument();
    });
    ...
    it('displays resolution when present', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Appeal was valid, ban removed')).toBeInTheDocument();
  ...
  describe('Review Modal', () => {
    it('opens resolution modal when Review button is clicked', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText('Resolve Appeal for user_one')).toBeInTheDocument();
      });
    });
    ...
    it('closes modal and reloads appeals on success', async () => {
      ...
      vi.clearAllMocks();
      const successButton = screen.getByRole('button', { name: 'Resolve Success' });
  ...
  describe('Error Handling', () => {
    it('displays error message when loading fails', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Failed to load appeals/)).toBeInTheDocument();
      });
    });
  ...
  describe('Accessibility', () => {
    it('has proper heading structure', () => {
      ...
      expect(screen.getByRole('heading', { name: 'Appeals Queue' })).toBeInTheDocument();
    });
