...
export function SyncBansModal({ open, onClose, channelId, onSuccess }: SyncBansModalProps) {
  ...
  const handleConfirmSync = async () => {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Failed to start sync. Please try again.'));
    ...
    if (syncProgress?.status === 'completed') {
      return (
        <div className="py-8 space-y-4">
          <Alert variant="success" title="Sync Completed Successfully">
          ...
          <div className="bg-muted p-4 rounded-lg space-y-3">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-muted-foreground">New Bans Added:</span>
                <span className="font-medium text-success-600 dark:text-success-400">
                  +{syncProgress.bans_added}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Already Existing:</span>
                <span className="font-medium">{syncProgress.bans_existing}</span>
              </div>
              <div className="flex justify-between border-t pt-2">
                <span className="font-medium">Total Processed:</span>
                <span className="font-medium">{syncProgress.total_processed}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      return (
        <div className="py-8 space-y-4">
          <Alert variant="info" title="Synchronizing Bans">
            Please wait while we sync bans from Twitch. This may take a few moments.
          </Alert>
      ...
      return (
        <div className="space-y-6">
          <Alert variant="warning" title="Confirm Sync">
            You are about to sync all bans from the Twitch channel "{twitchChannelName}". This will:
        ...
        <div className="space-y-6">
          <div>
            <p className="text-sm text-muted-foreground mb-4">
              Sync bans from your Twitch channel to keep your ban list up to date.
            </p>
            ...
            <div className="mt-4">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Enter the exact Twitch channel name (e.g., "twitchdev")
              </p>
            </div>
          </div>
          ...
          <ModalFooter>
            ...
            <Button type="submit" variant="primary" disabled={isSubmitting || !twitchChannelName.trim()}>
              Start Sync
            </Button>
          </ModalFooter>
        </div>
  ...
  return (
    <Modal
      ...
      size="lg"
      closeOnBackdrop={!isSubmitting && syncProgress?.status !== 'in_progress'}
    >
      {renderContent()}
    </Modal>
  );
