...
interface UseWatchPartyWebSocketReturn {
    sendCommand: (
        command: Omit<WatchPartyCommand, 'party_id' | 'timestamp'>,
    ) => void;
...
export function useWatchPartyWebSocket({
    ...
    const sendCommandRef = useRef<
        | ((command: Omit<WatchPartyCommand, 'party_id' | 'timestamp'>) => void)
        | null
    >(null);
    ...
    const connect = useCallback(async () => {
        ...
        try {
            ...
            ws.onmessage = event => {
                try {
                    const syncEvent = JSON.parse(
                        event.data,
                    ) as WatchPartySyncEvent;
    ...
    const sendReaction = useCallback(
        (emoji: string, videoTimestamp?: number) => {
            sendCommand({
                type: 'reaction',
                emoji,
