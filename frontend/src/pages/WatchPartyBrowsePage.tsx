...
import { getPublicWatchParties, getTrendingWatchParties, joinWatchParty } from '../lib/watch-party-api';
...
export function WatchPartyBrowsePage() {
  ...
  const handleJoinParty = async (inviteCode: string) => {
    try {
      ...
      showToast(err instanceof Error ? err.message : 'Failed to join watch party', 'error');
    } finally {
      ...
      <div className="bg-surface-secondary rounded-lg p-4 hover:bg-surface-tertiary transition-colors">
        <div className="flex items-start justify-between mb-3">
          ...
          <Button
            ...
            disabled={isJoining || participantCount >= party.max_participants}
            isLoading={isJoining}
          >
            {participantCount >= party.max_participants ? 'Full' : 'Join'}
          </Button>
        </div>
  ...
  if (loading) {
    return (
      <>
        <SEO title="Browse Watch Parties" description="Discover and join watch parties" />
        <Container>
          <div className="py-8 flex items-center justify-center">
            <div className="text-center">
              <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary-500 mx-auto"></div>
              <p className="mt-4 text-content-secondary">Loading watch parties...</p>
            </div>
          </div>
        </Container>
      </>
      ...
      <Container>
        <div className="py-8">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-3xl font-bold text-content-primary mb-2">Watch Parties</h1>
              <p className="text-content-secondary">Watch clips together with friends and community</p>
            </div>
          ...
          {trendingParties.length > 0 && (
            <section className="mb-8">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-xl font-semibold text-content-primary">Trending Now</h2>
              </div>
          ...
          {/* All public parties */}
          <section>
            <h2 className="text-xl font-semibold text-content-primary mb-4">Public Watch Parties</h2>
            {publicParties.length > 0 ? (
              ...
              <div className="text-center py-12 bg-surface-secondary rounded-lg">
                <p className="text-content-secondary mb-4">No public watch parties available</p>
                <Button onClick={handleCreateParty}>
                  <Plus className="w-4 h-4 mr-2" />
          ...
          <div className="mt-8 p-6 bg-surface-secondary rounded-lg">
            <h3 className="text-lg font-semibold text-content-primary mb-3">Join with Invite Code</h3>
            <p className="text-sm text-content-secondary mb-4">
              Have an invite code? Enter it below to join a private watch party.
            </p>
