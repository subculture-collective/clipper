...
describe('SubmitClipPage', () => {
    ...
    describe('Authentication and Karma Gate', () => {
        it('shows login prompt when not authenticated', () => {
            ...
            render(_jsx(SubmitClipPage, {}));
            expect(screen.getByText('Submit a Clip')).toBeInTheDocument();
            expect(screen.getByText('You must be logged in to submit clips.')).toBeInTheDocument();
        ...
        it('shows warning when user has insufficient karma', async () => {
            ...
            render(_jsx(SubmitClipPage, {}));
            await waitFor(() => {
                expect(screen.getByText(/You need 50 more karma points to submit clips/)).toBeInTheDocument();
            });
        });
        it('does not show warning when user has enough karma', async () => {
            ...
            render(_jsx(SubmitClipPage, {}));
            await waitFor(() => {
                expect(screen.queryByText(/more karma points to submit/)).not.toBeInTheDocument();
            });
        });
    ...
    describe('Form Submission', () => {
        ...
        it('shows loading state during submission', async () => {
            const user = userEvent.setup();
            mockSubmitClip.mockImplementation(() => new Promise((resolve) => setTimeout(resolve, 1000)));
            render(_jsx(SubmitClipPage, {}));
            const clipUrlInput = screen.getByLabelText(/Twitch Clip URL/);
            await user.type(clipUrlInput, 'https://clips.twitch.tv/TestClip123');
            const submitButton = screen.getByRole('button', {
