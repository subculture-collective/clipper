...
describe('AdminClipsPage', () => {
    ...
    describe('Clip Queue Loading', () => {
        it('should load and display clip moderation queue items', async () => {
            ...
            mockGetModerationStats.mockResolvedValue({
                ...
                data: {
                    ...
                    total_rejected: 0,
                    total_escalated: 0,
                    by_content_type: { clip: 1 },
                    by_reason: { inappropriate: 1 },
            ...
            expect(
                screen.getByText('Clip Moderation Queue')
            ).toBeInTheDocument();
        ...
        it('should display empty state when no clips are in the queue', async () => {
            mockGetModerationQueue.mockResolvedValue({
                success: true,
                data: [],
                meta: {
            ...
            await waitFor(() => {
                expect(
                    screen.getByText(/no clips in the moderation queue/i)
                ).toBeInTheDocument();
            });
        });
