...
export function AdminClipsPage() {
    ...
    const loadQueue = useCallback(async () => {
        try {
            ...
            const response = await getModerationQueue(
                statusFilter,
                'clip', // Fixed to clip content type
                50
            );
            setItems(response.data || []);
        } catch (err: unknown) {
            setError(getErrorMessage(err) || 'Failed to load clip moderation queue');
        } finally {
            setIsLoading(false);
        }
    }, [statusFilter]);
    ...
    const handleBulkApprove = useCallback(async () => {
        ...
        try {
            ...
            setSuccess(
                `${result.processed} of ${result.total} clips approved successfully!`
            );
            setSelectedIds(new Set());
            loadQueue();
            loadStats();
        } catch (err: unknown) {
            setError(getErrorMessage(err) || 'Failed to approve clips');
        }
    ...
    const handleBulkReject = useCallback(async () => {
        ...
        try {
            ...
            setSuccess(
                `${result.processed} of ${result.total} clips rejected successfully!`
            );
            setSelectedIds(new Set());
            loadQueue();
            loadStats();
        } catch (err: unknown) {
            setError(getErrorMessage(err) || 'Failed to reject clips');
        }
    }, [selectedIds, loadQueue, loadStats]);
    ...
    const handleApprove = async (itemId: string) => {
        try {
            ...
            setError(getErrorMessage(err) || 'Failed to approve clip');
        }
    };
    ...
    const handleReject = async () => {
        ...
        try {
            ...
            setError(getErrorMessage(err) || 'Failed to reject clip');
        }
    };
    ...
    // Filter stats to show only clip-related counts
    const clipStats = stats
        ? {
              total_pending: stats.by_content_type?.clip || 0,
        ...
        <Container className='py-8'>
            <div className='max-w-6xl mx-auto'>
                <div className='mb-6'>
                    ...
                    <p className='text-muted-foreground'>
                        Review and moderate flagged clips with bulk actions
                    </p>
                    <p className='text-muted-foreground text-sm mt-2'>
                        Keyboard shortcuts: A to approve • R to reject (when clips selected)
                    </p>
                </div>
                ...
                <Card className='p-6'>
                    {isLoading ? (
                        ...
                        <div className='py-12 text-center'>
                            <p className='text-muted-foreground'>
                                No clips in the moderation queue.
                            </p>
                        </div>
                    ) : (
                                    ...
                                    <div className='flex gap-4 items-start'>
                                        ...
                                        <div className='flex-1'>
                                            ...
                                            <div className='text-muted-foreground space-y-1 text-sm'>
                                                ...
                                                <p>
                                                    <span className='font-medium'>Clip ID:</span>{' '}
                                                    {item.content_id}
                                                </p>
                                                <p>
            ...
            {rejectModalOpen && (
                ...
                >
                    <div className='space-y-4'>
                        <p className='text-muted-foreground'>
                            Please provide a reason for rejecting this clip (optional).
                        </p>
                        <TextArea
