...
func (s *ModerationService) BanUser(ctx context.Context, communityID, moderatorID, targetUserID uuid.UUID, reason *string) error {
	...
	// Validate permission
	if err = s.validateModerationPermission(ctx, moderator, communityID, "ban"); err != nil {
		return err
	}
...
func (s *ModerationService) UnbanUser(ctx context.Context, communityID, moderatorID, targetUserID uuid.UUID) error {
	...
	// Validate permission
	if err = s.validateModerationPermission(ctx, moderator, communityID, "unban"); err != nil {
		return err
	}
...
func (s *ModerationService) GetBans(ctx context.Context, communityID, moderatorID uuid.UUID, page, limit int) ([]*models.CommunityBan, int, error) {
	...
	// Validate permission
	if err := s.validateModerationPermission(ctx, moderator, communityID, "view_bans"); err != nil {
		return nil, 0, err
	}
...
func (s *ModerationService) validateModerationPermission(ctx context.Context, moderator *models.User, communityID uuid.UUID, permissionType string) error {
	start := time.Now()
	var result string
	var reason string
