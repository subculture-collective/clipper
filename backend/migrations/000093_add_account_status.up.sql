...
-- Supports active (normal user), unclaimed (auto-created from Twitch), pending (being claimed), merged (account merged into another)
ALTER TABLE users
...
CHECK (account_status IN ('active', 'unclaimed', 'pending', 'merged'));
...
COMMENT ON COLUMN users.account_status IS 'Account status: active (registered user), unclaimed (auto-created from clip creator), pending (being claimed), merged (account merged into another account)';
