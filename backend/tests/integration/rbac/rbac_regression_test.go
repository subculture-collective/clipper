...
func setupRBACTestRouter(t *testing.T) *TestContext {
	...
	v1 := r.Group("/api/v1")
	{
		...
		// Watch party routes (auth required; return 404 for missing resources)
		watchParties := v1.Group("/watch-parties")
		{
			watchParties.POST("/:id/kick", middleware.AuthMiddleware(authService), notFoundHandler)
			watchParties.POST("/:id/end", middleware.AuthMiddleware(authService), notFoundHandler)
		}

		// Chat moderation routes (auth required; return 404 for missing resources)
		chat := v1.Group("/chat/channels")
		{
			chat.POST("/:id/ban", middleware.AuthMiddleware(authService), notFoundHandler)
			chat.POST("/:id/mute", middleware.AuthMiddleware(authService), notFoundHandler)
		}
		...
		// Admin operational routes (auth + admin/mod roles) returning 404 when resource is missing
		adminOps := v1.Group("/admin")
		adminOps.Use(middleware.AuthMiddleware(authService))
		adminOps.Use(middleware.RequireRole("admin", "moderator"))
		{
			adminOps.GET("/webhooks/dlq", notFoundHandler)
			adminOps.POST("/webhooks/dlq/:id/replay", notFoundHandler)
			adminOps.DELETE("/webhooks/dlq/:id", notFoundHandler)
			adminOps.POST("/discovery-lists", notFoundHandler)
			adminOps.DELETE("/discovery-lists/:id", notFoundHandler)
			adminOps.POST("/forum/threads/:id/lock", notFoundHandler)
			adminOps.POST("/forum/threads/:id/delete", notFoundHandler)
		}
	}
